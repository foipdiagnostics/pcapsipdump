...
int main(int argc, char *argv[])
{
      ...
      switch (c) {
         ...
         case 'R':
               if (strcasecmp(optarg,"none")==0){
                  ...
                  opt_t38only=1;
               }else if (strcasecmp(optarg,"all")==0 || strcasecmp(optarg,"rtp+rtcp")==0){
   ...
   if (verbosity>=2){
      printf("Working directory for output files: %s\n", opt_chdir);
   }
   if ((fname==NULL)&&(ifname==NULL)){
      ...
      " -v   Set verbosity level (2 = additional debug 3 = super verbose).\n"
      ...
      "  ** Note: Looks for SIP calls on UDP ports 5060 thru 5090\n"
      ,PCAPSIPDUMP_VERSION);
      return 1;
   }
   ...
   if (verbosity>=2){
      printf("Changed to working directory successfully.\n");
   }
   if (verbosity>=2){
      char cwd[1024];
   ...
   while((res=pcap_next_ex( handle, &pkt_header, &pkt_data)) >= 0){
   {
      ...
      if (header_ip->protocol==17){  //UPPROTO_UDP=17
         ...
         if (verbosity>=3){
            printf("save_this_rtp_packet: %d\n", save_this_rtp_packet);
            printf("Table size: %lu\n", ct->get_table_size());
            for (int i=0; i < ct->get_table_size(); i++) {
               for (int j=0; j < ct->table[i].ip_n; j++) {
                  printf("Call entry %d, IP %d: addr=%s, port=%u, ssrc=%u\n",
                         i, j, inet_ntoa(*(struct in_addr*)&ct->table[i].ip[j]),
                         ct->table[i].port[j], ct->table[i].ssrc[j]);

               }
            }

            struct in_addr ip_addr;
            ip_addr.s_addr=header_ip->daddr;
            printf("IP address: %s\n", inet_ntoa(ip_addr));

            int result=ct->find_ip_port_ssrc(
               header_ip->daddr, htons(header_udp->dest) & rtp_port_mask,
               get_ssrc(data, is_rtcp), &idx_leg, &idx_rtp);
            printf("find_ip_port_ssrc result: %d, idx_leg: %d, idx_rtp: %d\n", result, idx_leg, idx_rtp);
            printf("Calling find_ip_port_ssrc with addr: %u, port: %u, ssrc: %u\n",
                   header_ip->daddr, htons(header_udp->dest) & rtp_port_mask,
            ...
            }else if (htons(header_udp->source) >= 5060 && htons(header_udp->source) <= 5090 ||
                      htons(header_udp->dest) >= 5060 && htons(header_udp->dest) <= 5090){
               char caller[256];
               char called[256];
               char sip_method[256];
               ...
               if (s!=NULL && ((idx=ct->find_by_call_id(s,l))<0) && number_filter_matched){
                  if ((idx=ct->add(s,l,pkt_header->ts.tv_sec))<0){
                     ...
                     if ((strcmp(sip_method,"INVITE")==0)||(strcmp(sip_method,"OPTIONS")==0)||(strcmp(sip_method,"REGISTER")==0)){
                        ...
                        mkdir(str2,0700);
                        sprintf(str2,"%04d%02d%02d/%02d/%04d%02d%02d_%02d%02d%02d_%s_%s",
                           t->tm_year+1900,t->tm_mon+1,t->tm_mday,t->tm_hour,
   ...
   // If no packets are found, add a message to indicate this.
   if (res==-1) {
      printf("No packets found in the pcap file or error reading packets.\n");
   } else if (res==-2) {
      printf("End of file reached while reading the pcap file.\n");
   }
