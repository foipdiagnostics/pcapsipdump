...
unsigned long calltable::get_table_size()
{
   return table_size;
}

void calltable::set_verbosity(int level) {
    verbosity=level;
}
...
int calltable::add(char *call_id,unsigned long call_id_len,time_t time)
{
   ...
   for (i=0;i<table_size;i++){
    ...
    table[idx].is_used=1;
    if (verbosity>=3){
      printf("Adding call entry: idx = %u, call_id = %s, call_id_len = %lu\n", idx, call_id, call_id_len);
    }
    table[idx].had_t38=0;
    table[idx].had_bye=0;
...
int calltable::find_ip_port_ssrc(in_addr_t addr,unsigned short port,uint32_t ssrc,int *idx_leg,int *idx_rtp)
{
   int i_leg,i_rtp;
